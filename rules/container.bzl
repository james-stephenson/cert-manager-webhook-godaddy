...
load("@io_bazel_rules_docker//contrib:push-all.bzl", bundle_push = "container_push")
...
def multi_arch_container_push(
    ...
    native.alias(
        ...
        tags = tags,
        actual = select({
            go_platform_constraint(os = "linux", arch = arch): "bundle-%s-%s" % (name, arch)
            for arch in architectures
        }),
    )
